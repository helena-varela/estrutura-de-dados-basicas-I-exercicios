...
{	
	...
	while (atual != cauda)
	{
		...
		{
			// criando novoNo com conteudo novoElemento
			No<std::string>* novoNo = new No<std::string>(novoElemento);

			No<std::string>* vizinho_tras = atual->getAnterior();

			novoNo->setProximo(atual);
			novoNo->setAnterior(vizinho_tras);

			atual->setAnterior(novoNo);
			vizinho_tras->setProximo(novoNo);
